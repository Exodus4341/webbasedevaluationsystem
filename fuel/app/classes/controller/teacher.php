...
{
	public $template = 'teacher/template';

	public function before()
	{
		parent::before();

		if (Request::active()->controller !== 'Controller_Teacher' or ! in_array(Request::active()->action, array('login', 'logout')))
		{
			if (Auth::check())
			{
				if ( ! Auth::member(50))
				{
					Session::set_flash('error', e('You don\'t have access to the teacher panel'));
					Response::redirect('/');
				}
			}
			else
	...
	{
		// Already logged in
		Auth::check() and Response::redirect('teacher');
