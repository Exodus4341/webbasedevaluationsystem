         ...
         if($y == 0){
         	for ($i=0,$x = 1; $i < sizeof($category); $i++, $x++) { 
         		...
         		if(sizeof($evaluated) == 1){
                 	...
                 	$this->setX(120);
			  	 	$this->Cell($td[0],6,number_format(($sum/sizeof($evaluated)), 2, '.', '')." %",1,0,'C',true);
			  	 	$this->setX(150);
			  	 	$this->Cell($td[0],6,'',1,0,'C',true);
			  	 	$this->setX(180);
			 ...
			 /*Start of Conditon for Total Sum Rating per Subjects*/ 
			  $this->setX(90);
			  $this->Cell($td[$y],6,number_format(array_sum($percnt1[$y]), 2, '.', '')." %",1,0,'C',true);
			 /*Start of Conditon for Total Sum Rating per Subjects*/ 
