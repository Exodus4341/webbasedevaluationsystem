<ol class='breadcrumb'>
    <li><?php echo Html::anchor('teacher/archives', 'Archives'); ?></li>
            ...
            echo "<tr>
             ...
             for ($i=0,$x = 1; $i < sizeof($category); $i++, $x++) { 
                $percnt1[$y][$x] = ($evaluated[$y]['category_sum'.$x] / $questionsum[$y]['category'.$x]) * ($category[$i]['percentage']/100); 
                    ...
                    echo "<td>".Html::anchor('teacher/archives/student_list/'.$evaluated[$y]['subj_id'], '', array('class'=>'glyphicon glyphicon-list btn btn-default btn-xs', 'title'=>'Student List'))." ".Html::anchor('teacher/archives/result_evaluation/'.$evaluated[$y]['teacher_id'].'/'.$evaluated[$y]['subj_id'], '', array('class'=>'glyphicon glyphicon-th btn btn-success btn-xs', 'title'=>'View Result of evaluation'))."</td>";
                    echo "</tr>";
